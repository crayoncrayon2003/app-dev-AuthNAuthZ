#!/usr/bin/env python3
"""
case3_with_PKCE_setup_idm_ldap.py - OpenLDAP 初期設定スクリプト（IdM）
...
【Case3 の全体構成】
  ...
  Keycloak-IdP（ポート 8081）   ← OpenLDAP のユーザーで認証処理
        │ OIDC フェデレーション（認証委譲）
        ▼
  Keycloak-SP（ポート 8080）    ← クライアントが接続するサービス
        │ OIDC（認可コードフロー）
        ▼
  クライアント（client.py）
...
【実行順序】
  1. python case3_with_PKCE_setup_idm_ldap.py  ← このスクリプト
  2. python case3_with_PKCE_setup_idp.py
  3. python case3_with_PKCE_setup_sp.py
  4. python case3_with_PKCE_client.py

【実行方法】
  python case3_with_PKCE_   setup_idm_ldap.py
"""
...
if __name__ == "__main__":
    ...
    print(f"\n  次のステップ:")
    print(f"    python case3_with_PKCE_setup_idp.py\n")