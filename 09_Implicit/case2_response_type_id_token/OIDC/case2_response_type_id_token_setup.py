#!/usr/bin/env python3
"""
case2_response_type_id_token_setup.py - Keycloak 初期設定スクリプト
...
【処理内容】
  ...
  2. 管理者トークンの取得
  3. レルム (sample) の作成
  4. クライアント (oidc-implicit-id-token) の作成
  5. テストユーザーの作成
...
def create_client(token):
    ...
    resp = api_post(f"{KEYCLOAK_BASE}/admin/realms/{REALM_NAME}/clients", token, {
        "clientId":                  CLIENT_ID,
        "name":                      "OIDC インプリシットフロー (id_token) サンプル",
        "enabled":                   True,
...
if __name__ == "__main__":
    ...
    print(f"\n  次のステップ:")
    print(f"    python case2_response_type_id_token_client.py\n")