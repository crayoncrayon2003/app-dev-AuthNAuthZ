#!/usr/bin/env python3
"""
case1_response_type_code_without_PKCE_setup - Keycloak 初期設定スクリプト

ケース: response_type=code（OAuth2.0 認可コードフロー）

【OAuth2.0 と OIDC の違い】
  このスクリプトは OAuth2.0 専用です。
  OIDC 版との違い:
    - scope に openid を含めない
    - id_token は発行されない
    - クライアントID が異なる（oauth2-auth-code）

【処理内容】
  1. Keycloak 起動確認
  2. 管理者トークンの取得
  3. レルム (sample) の作成
  4. クライアント (oauth2-auth-code) の作成
  5. テストユーザーの作成

【実行方法】
  python case1_response_type_code_without_PKCE_setup.py
...
if __name__ == "__main__":
    ...
    print(f"\n  次のステップ:")
    print(f"    python case1_response_type_code_without_PKCE_client.py\n")