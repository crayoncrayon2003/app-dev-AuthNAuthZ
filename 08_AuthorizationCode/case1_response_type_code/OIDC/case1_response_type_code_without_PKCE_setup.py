#!/usr/bin/env python3
"""
case1_response_type_code_without_PKCE_setup.py - Keycloak 初期設定スクリプト

ケース: response_type=code（OIDC 認可コードフロー）

【処理内容】
  1. Keycloak 起動確認
  2. 管理者トークンの取得
  3. レルム (sample) の作成
  4. クライアント (oidc-auth-code) の作成
  5. テストユーザーの作成

【実行前提】
  docker compose up -d を実行し、Keycloak が起動していること
