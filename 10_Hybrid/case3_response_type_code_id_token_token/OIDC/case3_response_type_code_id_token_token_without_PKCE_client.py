#!/usr/bin/env python3
"""
case3_response_type_code_id_token_token_without_PKCE_client.py - OIDC ハイブリッドフロー クライアント

ケース: response_type=code id_token token

【認可レスポンス（フラグメント）で返るもの】
  code + id_token + access_token（全部）

【トークンエンドポイントで返るもの】
  access_token + id_token + refresh_token

【Case 1・Case 2 との違い】
  c_hash 検証（認可コードの改ざん検知）← Case 1 と同じ
  at_hash 検証（access_token の改ざん検知）← Case 2 にはない
  両方の検証が必要

【シーケンス】
  STEP 1.  state · nonce を生成
  STEP 2.  認可リクエスト URL を生成（response_type=code id_token token）
  STEP 3.  コールバックサーバ起動 & 認可URL 表示
  STEP 4.  フラグメント受信（code + id_token + access_token）
  STEP 5.  state 検証（CSRF 対策）
  STEP 6.  フラグメントの id_token 検証（JWT検証 + c_hash + at_hash）
  STEP 7.  トークンエンドポイントへ POST（認可コード → トークン交換）
  STEP 8.  トークンエンドポイントの id_token 検証（OIDC のみ）
  STEP 9.  UserInfo エンドポイント呼び出し（OIDC のみ）
  STEP 10. sub 一致確認（OIDC のみ）

【実行方法】
  python case3_response_type_code_id_token_token_without_PKCE_client.py
...
REALM             = "sample"
CLIENT_ID         = "oidc-hybrid-code-id-token-token"
...
class CallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/callback":
            ...
            print("  フラグメント（#code=...&id_token=...&access_token=...）はサーバに届きません。")
            ...
            print("  ポイント: code + id_token + access_token の全部がフラグメントで返されます\n")
...
def verify_fragment_id_token(id_token, code, access_token, nonce):
    print_step(6, "フラグメントの ID トークン検証")
    print("  検証項目: 署名 / iss / aud / exp / nonce / c_hash / at_hash\n")
    ...
    if not c_hash_in_token or c_hash_in_token != c_hash_computed:
        print("  [ERROR] c_hash 検証失敗")
        sys.exit(1)
    print_ok("c_hash 検証 OK")
    ...
    if not at_hash_in_token or at_hash_in_token != at_hash_computed:
        print("  [ERROR] at_hash 検証失敗")
        sys.exit(1)
    print_ok("at_hash 検証 OK")
...
def main():
    print("\n" + "="*60)
    print(" OIDC ハイブリッドフロー クライアント (PKCEなし)")
    print(" Case 3 - response_type=code id_token token")
    print("="*60)
    ...
    access_token_frag = _fragment_result.get("access_token", "")
    ...
    # STEP 6
    verify_fragment_id_token(id_token_frag, code, access_token_frag, nonce)

    # STEP 7
    token_resp = exchange_token(code)

    # STEP 8〜10
    id_token_payload = verify_token_endpoint_id_token(token_resp.get("id_token", ""), nonce)
