#!/usr/bin/env python3
"""
case3_response_type_code_id_token_token_with_PKCE_setup.py - Keycloak 初期設定スクリプト
...
【処理内容】
  ...
  2. 管理者トークンの取得
  3. レルム (sample) の作成
  4. クライアント (oidc-hybrid-code-id-token-token) の作成
  5. テストユーザーの作成

【実行方法】
  python case3_response_type_code_id_token_token_with_PKCE_setup.py
"""
...
def create_realm(token):
    ...
    resp = api_post(f"{KEYCLOAK_BASE}/admin/realms", token, {
        "realm":               REALM_NAME,
        "enabled":             True,
        "displayName":         "OIDC ハイブリッドフロー サンプルレルム",
        "sslRequired":         "none",
...
def create_client(token):
    ...
    resp = api_post(f"{KEYCLOAK_BASE}/admin/realms/{REALM_NAME}/clients", token, {
        "clientId":                  CLIENT_ID,
        "name":                      "OIDC ハイブリッドフロー (code id_token token) サンプル",
        "enabled":                   True,
    ...
    print(f"       standardFlow + implicitFlow : True (ハイブリッドフロー)")
    print(f"       PKCE : S256 / redirectUri : {REDIRECT_URI}")
...
if __name__ == "__main__":
    ...
    print(f"\n  次のステップ:")
    print(f"    python case3_response_type_code_id_token_token_with_PKCE_client.py\n")