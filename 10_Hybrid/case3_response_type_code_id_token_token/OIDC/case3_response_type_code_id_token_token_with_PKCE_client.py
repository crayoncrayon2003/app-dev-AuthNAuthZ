#!/usr/bin/env python3
"""
case3_response_type_code_id_token_token_with_PKCE_client.py - OIDC ハイブリッドフロー クライアント
...
【実行方法】
  python case3_response_type_code_id_token_token_with_PKCE_client.py
...
def verify_fragment_id_token(id_token, code, access_token, nonce):
    ...
    if not at_hash_in_token:
        print("  [ERROR] at_hash クレームがありません")
        sys.exit(1)
...
def main():
    ...
    for k, v in urllib.parse.parse_qsl(parsed.query):
        print(f"    &{k}={v}")
    print(f"\n  ポイント: response_type=code id_token token")
    print(f"    → フラグメントに code + id_token + access_token が全部返される")
    print(f"    → c_hash と at_hash の両方の検証が必要")
