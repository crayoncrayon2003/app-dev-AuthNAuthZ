#!/usr/bin/env python3
"""
case1_response_type_code_id_token_with_PKCE_setup.py - Keycloak 初期設定スクリプト
...
【処理内容】
  ...
  2. 管理者トークンの取得
  3. レルム (sample) の作成
  4. クライアント (oidc-hybrid-code-id-token) の作成
  5. テストユーザーの作成

【実行方法】
  python case1_response_type_code_id_token_with_PKCE_setup.py
"""
...
def create_client(token):
    ...
    resp = api_post(f"{KEYCLOAK_BASE}/admin/realms/{REALM_NAME}/clients", token, {
        "clientId":                  CLIENT_ID,
        "name":                      "OIDC ハイブリッドフロー (code id_token) サンプル",
        "enabled":                   True,
...
if __name__ == "__main__":
    ...
    print(f"\n  次のステップ:")
    print(f"    python case1_response_type_code_id_token_with_PKCE_client.py\n")