#!/usr/bin/env python3
"""
case1_response_type_code_id_token_with_PKCE_client.py - OIDC ハイブリッドフロー クライアント
...
【実行方法】
  python case1_response_type_code_id_token_with_PKCE_client.py
...
def main():
    ...
    for k, v in urllib.parse.parse_qsl(parsed.query):
        print(f"    &{k}={v}")
    print(f"\n  ポイント: response_type=code id_token")
    print(f"    → フラグメントに code + id_token が返される")
    print(f"    → id_token の c_hash で認可コードの改ざんを検知できる")
